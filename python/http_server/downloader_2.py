...
DOWNLOAD_LIST: List[Dict[str, str]] = [
    {
        "url": "http://192.168.208.199:62000/download/Docker_Course.pdf?rate=10000",
        "md5": "a8c879ee312ef952791982f4a5d508dc",
    },
    {
        "url": "http://192.168.208.199:62000/download/D3_2017.pdf?rate=1024",
        "md5": "be0541cc438ea46a16a0a762f18552e3",
    },
    {
        "url": "http://192.168.208.199:62000/download/Plotly.pdf?rate=1024",
        "md5": "be0541cc438ea46a16a0a762f18552e3",
    },
    {
        "url": "http://192.168.208.199:62000/download/D3.pdf?rate=1024",
        "md5": "d28c4d3844cc901d2a57e5937c0c727d",
    },
    {
        "url": "http://192.168.208.199:62000/download/Java.pdf?rate=1024",
        "md5": "a8c879ee312ef952791982f4a5d508dc",
    },
]
...
stop_event = signal_event = __import__("threading").Event()
...
def create_download_session(task_uuid: str) -> requests.Session:
    """
    Создает новую HTTP/TCP-сессию для одного скачивания.
    После завершения задачи она обязательно закрывается.
    """
    ...
    logger.debug("task_id=%s | Создана новая session для скачивания", task_uuid)
    return session
...
def download_and_validate(item: Dict[str, str], download_dir: Path) -> bool:
    ...
    session = create_download_session(task_uuid)
    ...
    try:
        ...
        try:
            session.close()
            logger.debug("task_id=%s | Session закрыта", task_uuid)
        except Exception as exc:
            logger.error("task_id=%s | Не удалось закрыть session: %s", task_uuid, exc)
...
def run_forever(download_list: List[Dict[str, str]], download_dir: Path, max_workers: int) -> None:
    ...
    logger.info(
        "Запуск непрерывного режима: постоянное число задач=%d, URL в пуле=%d",
        max_workers,
        len(download_list),
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="dl") as executor:
        ...
        def submit_next() -> None:
            ...
            total_submitted += 1
            logger.debug(
                "Отправлена новая задача. submitted=%d active=%d",
                total_submitted,
                len(futures),
            )
...
def main() -> int:
    ...
    logger.info("Параллельных задач: %s", MAX_WORKERS)
    logger.info("Файлов в пуле URL: %s", len(DOWNLOAD_LIST))
    logger.info("Режим соединений: новая TCP session на каждое скачивание")
