...
<storage>/<PREFIX>_<RUN_ID>_<SUFFIX>/
    <PREFIX>_<RUN_ID>_<name>_<SUFFIX>.csv          # метрики
    <PREFIX>_<RUN_ID>_<name>_<SUFFIX>.raw.txt      # сырые ответы
    <PREFIX>_<RUN_ID>_<name>_<SUFFIX>.summary.txt  # статистика
```
...
data/bench_2026-06-05_14h30m00-123456_run1/
    ...
    bench_2026-06-05_14h30m00-123456_lb_os_run1.summary.txt
...
- Число уникальных метрик и таблица «метрика → сколько раз получена».
...
Покрывают парсинг метрик, формирование имён, а также интеграционный сценарий:
реальный подъём mock-сервера, опрос, проверка CSV (включая `None` на месте
выпавших метрик), маркеров raw.txt и содержимого summary.txt.
