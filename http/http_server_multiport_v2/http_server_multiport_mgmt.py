...
class Rule:
    ...
    default_rate_kb: int = 100
    max_rate_kb: int = 1024
                    ...
                    for r in entry['rules']:
                        ...
                        if 'service' in resp:
                            rules.append(Rule(
                                ...
                                default_rate_kb=int(resp.get('default_rate_kb', 100)),
                                max_rate_kb=int(resp.get('max_rate_kb', 1024)),
                            ))
                        else:
    ...
    class RulesetHTTPHandler(http.server.BaseHTTPRequestHandler):
        ...
        def _handle_static_serve(self, rule: Rule, request_path: str, query: str):
            ...
            try:
                ...
                self.send_header('X-Rate-Limit', f'{rate_kb} KB/s')
                self.end_headers()
                chunk_size = 8192
